...
##NEED TO ADD AN EXCEPTION THAT DEALS WITH HOW POKEMON ARE HANDLED DURING SWITCHINS. POISON AND HP ALREADY LOST ARE NOT BEING TRACKED.
...
class pokemon:
    def __init__(self,species,level,trainer=False):
        ...
        self.lastUsedMove = False
        self.pursuitDamage = 0
        self.healthRestored = 0
        self.lastDamageDealt = 0
        self.lastDamage = 0
        self.stealthRockDamage = 0
        self.recoilDamage = 0
        ...
        self.suckerPunchFail = True


        ##important important important
        self.statCalculate()

    def forceMoves(self,moves):
        if len(moves) == 4:
            self.moves = []
            for nameOfMove in moves:
                self.moves.append([move(nameOfMove,self), moveInfo[nameOfMove]['pp']])
        else:
            raise ValueError
    ...
    def resettables(self,dontChangeSleep=False):
        ...
        if self.lastUsedMove == 'Fury Cutter': ##each use of fury cutter will keep self.lastUsedMove seeing Fury Cutter
            self.furyCutterCount += 1          ##so will raise the count continuously
        else:                                  ##it is set to false at the end of every turn so that if the user is sent to sleep
            self.furyCutterCount = 0           ##they will be guaranteed to have their furyCutterCount reset. clever, isn't it?
        self.lastUsedMove = False
...
class trainer:
    def __init__(self,name,trainerClass,spriteName=False,powerLevel='regular'):
    ...
    def removeFromTeam(self,pointer):
        ...
        return temp
    def getIndexesNotFainted(self): ##this and the next function are primarily used for the move Roar
        res = []
        for x in self.team:
            if self.team[x].fainted == False and x != str(self.active):
                res.append(x)
        return res
    def getListNotFainted(self):
        res = []
        for x in self.team:
            if self.team[x].fainted == False and x != str(self.active):
                res.append(team[x])
...
class Arena:
    def __init__(self,trainer1,trainer2,receiveInfo=False):
        self.removeList = []
        self.userTrainer = trainer1
        self.userTrainer.active = 0
        self.enemyTrainer = trainer2
        self.enemyTrainer.active = 0
        print(self.userTrainer.team)
        print(self.enemyTrainer.team)
        ...
        self.stealthRock = {self.userTrainer : False, self.enemyTrainer : False}
        self.stickyWebs = {self.userTrainer : False, self.enemyTrainer : False}
    ...
    def stealthRockModifiers(self,target):
        M = 1
        for Type in target.types:
            if 'Rock' in weakness[Type]:
                M = M * 2
            elif 'Rock' in resistance[Type]:
                M = M/2
        return M
    ...
    def processTurn(self,userInput,enemyInput):
        ...
        print(userInput[1])
        try:
            print(userInput[1].name)
        except:
            print(userInput[0])
        try:
            print(enemyInput[1].name)
        except:
            print(userInput[1])
        ...
        if userInput[0] == 'attack' and enemyInput[0] == 'attack': ## need to decide which goes first
            ...
            if userInput[1].priority > enemyInput[1].priority: 
                ...
                if userSpeed > enemySpeed:
                    if userInput[1].name in ['Roar','Whirlwind'] and enemyInput[1].name in ['Roar','Whirlwind']:
                        if len(self.enemyTrainer.getListNotFainted()) == 0:
                            order = (userInput,enemyInput)
                        else:
                            order = (userInput)
                    elif not self.trickRoom: ##trick room is a move that makes slower pokemon move first
                        order = (userInput,enemyInput)
                    else:
                        order = (enemyInput,userInput)
                elif userSpeed < enemySpeed:
                    if userInput[1].name in ['Roar','Whirlwind'] and enemyInput[1].name in ['Roar','Whirlwind']:
                        if len(self.userTrainer.getListNotFainted()) == 0:
                            order = (enemyInput,userInput)
                        else:
                            order = (enemyInput)
                    elif not self.trickRoom:
                        order = (enemyInput,userInput)
                    else:
                        order = (userInput,enemyInput)
                    ...
                    if userInput[1].name in ['Roar','Whirlwind'] and enemyInput[1].name in ['Roar','Whirlwind']:
                        ...
                        order = random.shuffle([enemyInput,userInput])
                    if order[0][2] == 'user': ##speed tie sucker punches decided here
                        self.userTrainer.team[str(self.userTrainer.active)].suckerPunchFail = False
                    elif order[0][2] == 'enemy':
                        self.enemyTrainer.team[str(self.enemyTrainer.active)].suckerPunchFail = False
                        
                    order = tuple(x)
                    
            ##########SUCKER PUNCH CHECK##########
            if userInput[1].name == 'Sucker Punch' and enemyInput[1].name != 'Sucker Punch' and enemyInput[1].category in ['Physical','Special']:
                self.userTrainer.team[str(self.userTrainer.active)].suckerPunchFail = False
            elif enemyInput[1].name == 'Sucker Punch' and userInput[1].name != 'Sucker Punch' and userInput[1].category in ['Physical','Special']:
                self.enemyTrainer.team[str(self.enemyTrainer.active)].suckerPunchFail = False
            elif enemyInput[1].name == userInput[1].name == 'Sucker Punch':
                if userSpeed > enemySpeed:
                    self.userTrainer.team[str(self.userTrainer.active)].suckerPunchFail = False
                elif enemySpeed > userSpeed:
                    self.enemyTrainer.team[str(self.enemyTrainer.active)].suckerPunchFail = False
                ##speed tie sucker punches are decided above
            ########END SUCKER PUNCH CHECK########
                
        elif userInput[0] != enemyInput[0]: ## i.e. if one is switching and one isn't
            if userInput[0] == 'switch': ## is user the one switching?
                if enemyInput[1].name == 'Pursuit': ##pursuit chases a pokemon if its switching so that it hits them before they can go.
                    order = (enemyInput,userInput)
                else:
                    order = (userInput,enemyInput)
            else: ## so enemy is switching
                if userInput[1].name == 'Pursuit':
                    order = (userInput,enemyInput)
                else:
                    order = (enemyInput,userInput)
            ...
            if self.userTrainer.team[str(self.userTrainer.active)].stats[5] > self.enemyTrainer.team[str(self.enemyTrainer.active)].stats[5]:
                ...
                order = tuple(x)
        q = serviceQueue()
        order = list(order) ##i realized after coding all this that order needed to be a list so now ive got to change it here. whoops. (blame the move roar which needs to append stuff lol)
        for x in order:
            badFlag = 0
            ...
            if x[2] == 'user' and self.faintedMidBattle[0]:
                ...
                if x[2] == 'user': ## I could compress k=genfunc(...); q.add(k) into one line but they are kept separate for clarity
                    if self.userTrainer.team[str(self.userTrainer.active)].pursuitDamage > self.userTrainer.team[str(self.userTrainer.active)].hp:
                        continue
                ...
                ##THE FOLLOWING TWO ARE ENTRY HAZARDS. THEY ACTIVATE WHEN A POKEMON SWITCHES IN AND THEY ARE ACTIVE.
                    ##CHECK FOR STEALTH ROCK
                    if self.stealthRock[self.userTrainer]:
                        pkmn = self.userTrainer.team[str(x[1])]
                        damage = int(pkmn.stats[0]/8 * self.stealthRockModifiers(pkmn))
                        q.add(genFunc(self.app.runText,pkmn.name + ' took damage from the floating rocks!'))
                        if damage >= pkmn.hp:
                            damage = pkmn.hp
                            k = genFunc(self.app.HPUpdateUser,0,1,0,True)
                            q.add(k)
                            self.faintedMidBattle[1] = True
                            q.add(genFunc(self.app.runText,pkmn.name + ' fainted!'))
                        else:
                            pkmn.stealthRockDamage = damage
                            k = genFunc(self.app.HPUpdateUser,pkmn.hp-damage,1,0,True)
                            q.add(k)

                    ##CHECK FOR STICKY WEBS
                    if self.stickyWebs[self.userTrainer] and 'Flying' not in self.userTrainer.team[str(x[1])].types:
                        pkmn = self.userTrainer.team[str(x[1])]
                        if pkmn.modifiers[5] > -6:
                            q.add(genFunc(pkmn.changeModifier,'speed',-1))
                            q.add(genFunc(self.app.runText,pkmn.name + "'s speed fell!"))
                    ...
                    q.add(k)
                    q.add(self.app.throwStartPokeball)
                    q.add(genFunc(self.app.runText,self.userTrainer.team[str(x[1])].name + ' was dragged out!'))
                    tempActiveUserTrainer = str(x[1])
                    ...
                    if self.enemyTrainer.team[str(self.enemyTrainer.active)].pursuitDamage > self.enemyTrainer.team[str(self.enemyTrainer.active)].hp:
                        continue
                    ...
                    if self.stealthRock[self.enemyTrainer]:
                        pkmn = self.enemyTrainer.team[str(x[1])]
                        damage = int(pkmn.stats[0]/8 * self.stealthRockModifiers(pkmn))
                        q.add(genFunc(self.app.runText,"Foe's " + pkmn.name + ' took damage from the floating rocks!'))
                        if damage >= pkmn.hp:
                            damage = pkmn.hp
                            k = genFunc(self.app.HPUpdateEnemy,0,1,0,True)
                            q.add(k)
                            self.faintedMidBattle[1] = True
                            q.add(genFunc(self.app.runText,"Foe's " + pkmn.name + ' fainted!'))
                        else:
                            pkmn.stealthRockDamage = damage
                            k = genFunc(self.app.HPUpdateEnemy,pkmn.hp-damage,1,0,True)
                            q.add(k)

                    if self.stickyWebs[self.enemyTrainer] and 'Flying' not in self.enemyTrainer.team[str(x[1])].types:
                        pkmn = self.enemyTrainer.team[str(x[1])]
                        if pkmn.modifiers[5] > -6:
                            q.add(genFunc(pkmn.changeModifier,'speed',-1))
                            q.add(genFunc(self.app.runText,"Foe's " + pkmn.name + "'s speed fell!"))
                    ...
                    q.add(k)
                    q.add(self.app.enemyPokemonSendOut)
                    q.add(genFunc(self.app.runText,"Foe's " + self.enemyTrainer.team[str(x[1])].name + ' was dragged out!'))
                    STATUSCONDITION = self.enemyTrainer.team[str(x[1])].statusCondition
                    tempActiveEnemyTrainer = str(x[1])
                    if STATUSCONDITION != None:
                        q.add(self.app.setEnemyStatus)
                ...
                ##
                    ...
                    ##add switching to end of turn if faints
...
##                elif user.confused == True and badFlag != 1:
##                    q.add(genFunc(self.app.runText,u+user.name+' is confused!'))
##                    if r.choice[0,1,2] == 0:
##                        badFlag = 1
##                        damage = findDamage(user,user,move('<Confused>',None))
##                        q.add(genFunc(self.app.runText,u+user.name+' hurt itself in its confusion!'))
##                        user.confusedDamage = damage
##                        if damage[0] >= targ.hp:
##                            if x[2]=='user':
...
##                                q.add(k)
                if badFlag == 1:
                    user.lastUsedMove = False
                    
                elif not UserFainted and badFlag == 0:
                    ...
                    if x[1].specifier == 'multihit':
                        ...
                        loop = 2
                    if x[1].name in ['Freeze Shock','Solar Beam'] and user.twoTurnMove == None:
                        ...
                        for hit in range(0,loop):
                            ...
                            if x[1].name == 'Sucker Punch':
                                if user.suckerPunchFail:
                                    q.add(genFunc(self.app.runText,'But it failed!'))
                                    continue
                            if loop > 1:
                            ...
                            if self.doesMoveHit(x[1],user,targ):
                                user.lastMoveUsed = x[1].name
                                numberHit += 1
                                if targ.lastUsedProtect and x[1].specifier != 'unaffected by protect':
                                ...
                                if x[1].category != 'Status':
                                    ...
                                    if damage[1] == 0:
                                        ...
                                        if x[1].name in ['Air Slash','Clear Smog','Dark Pulse','Dragon Rush','Drain Punch','Extrasensory','Fake Out','Fire Fang','Giga Drain','Ice Fang','Icicle Crash','Iron Head','Rapid Spin','Rock Slide','Thunder Fang','Waterfall','Zen Headbutt']:
                                            ##These moves have effects, such as flinching or stat drop or HP restoration, that mean that they need to be activated early.
                                        ...
                                        print('HEALTH RESTORED: ',targ.healthRestored)
                                        if damage[0] >= targ.hp - loopDamage + targ.healthRestored - targ.stealthRockDamage - targ.recoilDamage: ##loop damage is for looping moves that deal damage across multiple hits, healthRestored is for moves like rest drainpunch or gigadrain that if not accounted for might incorrectly call a KO animation
                                            user.lastDamageDealt = targ.hp
                                            if x[2]=='user':
                                                H = 'regular'
                                                if damage[1] == 2:
                                                    H = 'super'
                                                    ...
                                                    H = 'weak'
                                                k = genFunc(self.app.HPUpdateEnemy,0,1,0,False,H)
                                                q.add(k)
                                            else:
                                                H = 'regular'
                                                if damage[1] == 2:
                                                    ...
                                                    H = 'weak'
                                                k = genFunc(self.app.HPUpdateUser,0,1,0,False,H)
                                                q.add(k)
                                            ...
                                            if x[1].specifier == 'recoil':
                                                q.add(genFunc(self.app.runText,u+user.name+' took damage from recoil!'))
                                                if USERNAME == 'user':
                                                    f = self.app.HPUpdateUser
                                                else:
                                                    f = self.app.HPUpdateEnemy
                                                user.recoilDamage = int(damage[0]/3)
                                                if user.recoilDamage >= user.hp - targ.lastDamageDealt:
                                                    q.add(genFunc(f,0,1,0,True))
                                                    if USERNAME == 'user':
                                                        self.faintedMidBattle[1] = True
                                                        q.add(genFunc(self.app.runText,user.name + ' fainted!'))
                                                        q.add(user.modifierReset)
                                                        q.add(self.app.userPokemonFaint)
                                                    else:
                                                        self.faintedMidBattle[0] = True
                                                        q.add(genFunc(self.app.runText,"Foe's " + user.name + ' fainted!'))
                                                        q.add(user.modifierReset)
                                                        q.add(self.app.enemyPokemonFaint)
                                                else:
                                                    q.add(genFunc(f,user.hp - targ.lastDamageDealt - user.recoilDamage,1,0,True))
                                            ...
                                            if damage[1] == 2:
                                                ...
                                                H = 'weak'
                                            if x[2] == 'user':
                                                k = genFunc(self.app.HPUpdateEnemy,targ.hp-damage[0]-loopDamage,1,0,False,H)
                                            else:
                                                k = genFunc(self.app.HPUpdateUser,targ.hp-damage[0]-loopDamage,1,0,False,H)
                                            q.add(k)
                                            k = genFunc(targ.takeDamage,damage[0])
                                            q.add(k)
                                            if damage[1] == 1:
                                    ...
                                    if user.taunt:
                                        ...
                                        continue
                                    elif x[1].name in ['Roar','Whirlwind']:
                                        res = targ.trainer.getIndexesNotFainted()
                                        if len(res) == 0:
                                            q.add(genFunc(self.app.runText,'But it failed!'))
                                        else:
                                            order.append(['switch',int(r.choice(res)),TARGNAME+'-roar']) ##so that some different text is played, i'm using a different ID; i.e. user-roar, enemy-roar, etc.
                                    elif x[1].name in ['Heal Bell','Rest','Roost','Slack Off','Synthesis','Sleep Powder','Spore','Hypnosis','Clear Smog','Protect','Taunt']:
...
################################################################################
    ...
    def hurtByBurn(self,targ):
        ...
        q = serviceQueue()
        numTasks = 2
        if not targ.fainted and targ.statusCondition == 'burned' and (targ.pokemonID == self.getUserPokemonID() or targ.pokemonID == self.getEnemyPokemonID()):
    ...
    def hurtByBadPoison(self,targ):
        ...
        q = serviceQueue()
        numTasks = 2
        if not targ.fainted and targ.statusCondition == 'badly poisoned' and (targ.pokemonID == self.getUserPokemonID() or targ.pokemonID == self.getEnemyPokemonID()):
            try:
    ...
    def hurtByPoison(self,targ):
        ...
        q = serviceQueue()
        numTasks = 2
        if not targ.fainted and targ.statusCondition == 'poisoned' and (targ.pokemonID == self.getUserPokemonID() or targ.pokemonID == self.getEnemyPokemonID()):
            if targ.pokemonID == self.userTrainer.team[str(self.userTrainer.active)].pokemonID:
                j = ''
            else:
                j = "Foe's "
...
#############################################################
    ...
    def sendInIfFainted(self):
        ...
        if self.userTrainer.team[str(self.userTrainer.active)].fainted:
            ...
            q.add(self.checkUserStealthRock)
            totalPokemon = 0
            totalFainted = 0
            for x in range(6):
                if self.userTrainer.team[str(x)] == None:
                    break
                elif self.userTrainer.team[str(x)].fainted:
                    totalFainted += 1
                totalPokemon += 1
            if totalPokemon == totalFainted:
                self.app.endBattle('lose')
                return
        ...
        if self.enemyTrainer.team[str(self.enemyTrainer.active)].fainted:
            ...
            for x in range(6):
                print(self.enemyTrainer.team)
                if self.enemyTrainer.team[str(x)] == None:
                    break
                elif self.enemyTrainer.team[str(x)].fainted:
                    totalFainted += 1
                print(totalFainted,totalPokemon)
                totalPokemon += 1
            if totalPokemon == totalFainted:
                self.app.endBattle('win')
                return
            while True:
            ...
            q.add(self.checkEnemyStealthRock)
        print(q.q)
    ...
    def checkUserStealthRock(self):
        doSomething = False
        if self.stealthRock[self.userTrainer]:
            doSomething = True
            pkmn = self.userTrainer.team[str(self.userTrainer.active)]
            damage = int(pkmn.stats[0]/8 * self.stealthRockModifiers(pkmn))
            self.app.runText(pkmn.name + ' took damage from the floating rocks!')
            if damage >= pkmn.hp:
                damage = pkmn.hp
                self.app.HPUpdateUser(0,1,0,True)
                self.app.runText(pkmn.name + ' fainted!')
            else:
                pkmn.stealthRockDamage = damage
                self.app.HPUpdateUser(pkmn.hp-damage,1,0,True)

        if self.stickyWebs[self.userTrainer] and 'Flying' not in self.userTrainer.team[str(self.userTrainer.active)].types:
            doSomething = True
            pkmn = self.userTrainer.team[str(x[1])]
            if pkmn.modifiers[5] > -6:
                pkmn.changeModifier('speed',-1)
                self.app.runText(pkmn.name + "'s speed fell!")

        if not doSomething:
            self.app.taskFinished = True

    def checkEnemyStealthRock(self):
        doSomething = False
        if self.stealthRock[self.enemyTrainer]:
            doSomething = True
            pkmn = self.enemyTrainer.team[str(self.enemyTrainer.active)]
            damage = int(pkmn.stats[0]/8 * self.stealthRockModifiers(pkmn))
            self.app.runText("Foe's " + pkmn.name + ' took damage from the floating rocks!')
            if damage >= pkmn.hp:
                damage = pkmn.hp
                self.app.HPUpdateEnemy(0,1,0,True)
                self.app.runText("Foe's " + pkmn.name + ' fainted!')
            else:
                pkmn.stealthRockDamage = damage
                self.app.HPUpdateEnemy(pkmn.hp-damage,1,0,True)

        if self.stickyWebs[self.enemyTrainer] and 'Flying' not in self.enemyTrainer.team[str(self.enemyTrainer.active)].types:
            doSomething = True
            pkmn = self.enemyTrainer.team[str(self.enemyTrainer.active)]
            if pkmn.modifiers[5] > -6:
                pkmn.changeModifier('speed',-1)
                self.app.runText("Foe's " + pkmn.name + "'s speed fell!")
    ...
    def checkEnemySwitch(self,reset=0):
        ...
        if not reset and self.emergencyEnemySwitch == '':
            ...
            self.app.taskFinished = False
            self.app.runText(self.getEnemyNameAndClass() + ' has sent out ' + self.enemyTrainer.team[str(self.enemyTrainer.active)].name + '!')
...
#############################################################
    ...
    def findDamage(self,user,target,move):
        ...
        if move.name == 'Punishment':
            for mod in target.modifiers:
                if mod > 0:
                    P += 20 * mod
        if move.name == 'Hex' and (target.statusCondition in ['poisoned','badly poisoned','burned','freeze','paralyzed','asleep']):
            P = 130
        elif move.name == 'Electro Ball': ##different modifiers depneding ons peed ratio
            ...
            furyCutterDict = {1:80,0:40}
            if user.furyCutterCount in furyCutterDict:
                P = furyCutterDict[user.furyCutterCount]
            else:
                P = 160

        elif move.name in ['Avalance','Revenge'] and user.lastDamage > 0:
            P = 120

        elif move.name == 'Gyro Ball':
            P = min(150,int(25*target.stats[5]/user.stats[5]))
        
        if move.category == 'Physical':
            A = user.stats[1]
            D = target.stats[2]
            if move.name == 'Foul Play': ##foul play uses foe's attack agaisnt them.
                A = target.stats[1]
    ...
    def combinedModifiers(self,user,target,move):
        ...
        if move.category == 'Physical':
            ...
            if move.name not in ['Chip Away'] or not self.criticalHit:
                Modifier = Modifier / stages[str(target.modifiers[2])]
        else:
            x = stages[str(target.modifiers[4])]
            if self.criticalHit:
                x = 1
            Modifier = Modifier * stages[str(user.modifiers[3])] / x
    ...
    def typeModifiers(self,user,target,move):
        ...
        for Type in target.types:
            if move.type in immune[Type]:
                return (0,0)
            elif Type == 'Water' and move.name == 'Freeze Dry':
                Modifier = Modifier * 2
                superEffective = True 
                ...
                if move.name == 'Freeze Dry' and Type.lower() == 'water':
                    pass
                else:
                    Modifier = Modifier / 2
        ...
        chance = 16
        if move.name in ['Blaze Kick','Cross Poison','Drill Run','Leaf Blade','Night Slash','Psycho Cut','Shadow Claw','Slash','Stone Edge']:
            chance = 8
        if r.randint(0,chance) == 0 or move.name in ['Frost Breath']:
            self.criticalHit = True
            Modifier = Modifier * 2

        if target.minimize and move.name in ['Dragon Rush']:
            Modifier = Modifier * 2
...
class move:
    def __init__(self,name,pokemon):
        global moveInfo
        functions = {
            '<Confused>':self.doesNothing,
            
            #A
            'Acrobatics':self.doesNothing,
            'Aerial Ace':self.doesNothing,
            'Agility':self.rockPolish, ##effects are functionally the same as rock polish
            'Air Slash':self.rockSlide, ##effects are functionally the same as rock slide
            'Amnesia':self.amnesia,
            'Aqua Jet':self.doesNothing,
            'Aqua Tail':self.doesNothing,
            'Aura Sphere':self.doesNothing,
            'Avalanche':self.doesNothing,

            #B
            'Blaze Kick':self.blazeKick,
            'Blizzard':self.icePunch, ##effects are functionally the same as ice punch
            'Bone Rush':self.doesNothing,
            'Brave Bird':self.doesNothing,
            'Brick Break':self.doesNothing,
            'Bullet Punch':self.doesNothing,
            'Bullet Seed':self.doesNothing,
            ...
            'Cross Poison':self.sludgeWave, ##effects are functionally the same as sludge wave

            #D
            'Dark Pulse':self.darkPulse,
            'Dazzling Gleam':self.doesNothing,
            'Discharge':self.discharge,
            'Double Edge':self.doesNothing,
            'Double Team':self.doubleTeam,
            'Dual Chop':self.doesNothing,
            'Dragon Claw':self.doesNothing,
            'Draco Meteor':self.dracoMeteor,
            'Dragon Dance':self.dragonDance,
            'Dragon Pulse':self.doesNothing,
            'Dragon Rush':self.darkPulse, ##effects are functionally the same as dark pulse
            'Drain Punch':self.drainPunch,
            'Drill Peck':self.doesNothing,
            'Drill Run':self.doesNothing,
            
            #E
            'Earth Power':self.energyBall, ##effects are functionally the same as energy ball
            'Earthquake':self.doesNothing,
            'Electro Ball':self.doesNothing,
            'Energy Ball':self.energyBall,
            'Extrasensory':self.extrasensory,
            'Extreme Speed':self.doesNothing,
            
            #F
            'Facade':self.doesNothing,
            'Fake Out':self.fakeOut,
            'Fire Blast':self.firePunch, ##effects are functionally the same as fire punch
            'Fire Fang':self.fireFang,
            'Fire Punch':self.firePunch,
            'Flamethrower':self.firePunch, ##effects are functionally the same as fire punch
            'Flare Blitz':self.doesNothing,
            'Flash Cannon':self.flashCannon,
            'Focus Blast':self.flashCannon, ##effects are functionally the same as flash cannon
            'Foul Play':self.doesNothing,
            'Freeze-Dry':self.icePunch, ##effects are functionally the same as ice punch
            'Freeze Shock':self.freezeShock,
            'Frost Breath':self.doesNothing,
            'Fury Cutter':self.doesNothing,
            'Fusion Bolt':self.doesNothing,

            #G
            'Giga Drain':self.gigaDrain,
            'Growth':self.growth,
            'Gunk Shot':self.gunkShot,
            'Gyro Ball':self.doesNothing,

            #H
            'Hammer Arm':self.hammerArm,
            'Heal Bell':self.healBell,
            'Heat Wave':self.firePunch, ##effects are functionally the same as fire punch
            'Hex':self.doesNothing,
            'Hibernate':self.hibernate,
            'High Horsepower':self.doesNothing,
            'Hone Claws':self.honeClaws,
            'Hurricane':self.doesNothing,
            'Hydro Pump':self.doesNothing,
            'Hyper Voice':self.doesNothing,
            'Hypnosis':self.spore,

            #I
            'Ice Beam':self.icePunch, ##effects are functionally the same as ice punch
            'Ice Fang':self.iceFang,
            'Ice Punch':self.icePunch,
            'Ice Shard':self.doesNothing,
            'Icicle Crash':self.rockSlide, ##effects are functionally the same as rock slide
            'Icicle Spear':self.doesNothing,
            'Iron Head':self.rockSlide, ##effects are functionally the same as rock slide
            'Iron Tail':self.ironTail,
            'Iron Defense':self.ironDefense,

            #L
            'Leaf Blade':self.doesNothing,
            'Leaf Storm':self.dracoMeteor, ##effects are functionally the same as draco meteor
            'Light Screen':self.lightScreen,
            'Liquidation':self.crunch, ##effects are functionally the same as crunch

            #M
            'Mach Punch':self.doesNothing,
            'Magical Leaf':self.doesNothing,
            'Magnet Rise':self.magnetRise,
            'Megahorn':self.doesNothing,
            'Metal Claw':self.metalClaw,
            'Meteor Mash':self.meteorMash,
            'Minimize':self.minimize,
            'Moonblast':self.moonblast,
            'Mystical Fire':self.mysticalFire,

            #N
            'Nasty Plot':self.nastyPlot,
            'Night Slash':self.doesNothing,

            #O
            'Overheat':self.dracoMeteor, ##effects are functionally the same as draco meteor

            #P
            'Pin Missile':self.doesNothing,
            'Play Rough':self.playRough,
            'Poison Jab':self.sludgeBomb, ##effects are functionally the same as sludge bomb
            'Power Whip':self.doesNothing,
            'Protect':self.protect,
            'Psychic':self.psychic,
            'Psycho Cut':self.doesNothing,
            'Psyshock':self.doesNothing,
            'Punishment':self.doesNothing,
            'Pursuit':self.doesNothing,

            #R
            'Rapid Spin':self.rapidSpin,
            'Revenge':self.doesNothing,
            'Reflect':self.reflect,
            'Rest':self.rest,
            'Roar':self.doesNothing,
            'Rock Polish':self.rockPolish,
            'Rock Slide':self.rockSlide,
            'Roost':self.synthesis, ##effects are functionally the same as synthesis
            'Rock Blast':self.doesNothing,

            #S
            'Scald':self.scald,
            'Scary Face':self.scaryFace,
            'Screech':self.screech,
            'Seed Bomb':self.doesNothing,
            'Shadow Ball':self.shadowBall,
            'Shadow Claw':self.doesNothing,
            'Shadow Sneak':self.doesNothing,
            'Shell Smash':self.shellSmash,
            'Signal Beam':self.doesNothing,
            'Slack Off':self.synthesis,
            'Slash':self.doesNothing,
            'Sleep Powder':self.spore, ##functionall the same as spore
            'Sludge Bomb':self.sludgeBomb,
            'Sludge Wave':self.sludgeWave,
            'Smart Strike':self.doesNothing,
            'Solar Beam':self.solarBeam,
            'Spore':self.spore,
            'Stealth Rock':self.stealthRock,
            'Steel Wing':self.steelWing,
            'Sticky Webs':self.stickyWebs,
            'Stone Edge':self.doesNothing,
            'Sucker Punch':self.doesNothing,
            'Superpower':self.superpower,
            'Surf':self.doesNothing,
            ...
            #W
            'Waterfall':self.darkPulse, ##effects are functionally the same as dark pulse
            'Water Shuriken':self.doesNothing,
            'Whirlwind':self.doesNothing,
            'Will-O-Wisp':self.willOWisp,
            'Wish':self.wish,

            #X
            'X-Scissor':self.doesNothing,
            ...
            'Zen Headbutt' : self.zenHeadbutt,
            }
        
        moveSpecifier = {'Bone Rush' : 'multihit','Brave Bird':'recoil','Bullet Seed' : 'multihit','Double Edge':'recoil','Dual Chop':'twohit','Flare Blitz':'recoil','Icicle Spear':'multihit','Pin Missile':'multihit','Rock Blast':'multihit','Water Shuriken':'multihit'}
        for x in ['Wish','Whirlwind','Trick Room','Synthesis','Swords Dance','Shell Smash','Sticky Webs','Stealth Rock','Slack Off','Roost','Rest','Reflect','Nasty Plot','Light Screen','Hibernate','Double Team','Calm Mind','Amnesia','Agility']:
            moveSpecifier[x] = 'unaffected by protect'
        self.name = name
    ...
    def statChange(self,arena,targ,stat,modifier,dontSayIfNothingHappens=False):
        ...
        if x == 'ALLOK':
            ...
            if targ.pokemonID == arena.enemyTrainer.team[str(arena.enemyTrainer.active)].pokemonID:
                u = "Foe's "
            if h == 'rose':
                T = threading.Thread(target=genFunc(playsound,'sounds/statrise.mp3'))
            elif h == 'fell':
                T = threading.Thread(target=genFunc(playsound,'sounds/statfall.mp3'))
            T.start()
            arena.app.runText(u + targ.name + "'s " + stat + " " + h + d[modifier]+'!')
        else:
            if modifier > 0:
                h = 'higher'
            elif modifier < 0:
    ...
    def blazeKick(self,arena,user,targ):
        ...
        if r.uniform(0,1) > 0.9:
            if targ.types[1].lower() != 'fire' and targ.types[0].lower() != 'fire' and targ.statusCondition == None:
                ...
                f()
                arena.endOfTurnQueue.add(arena.hurtByBurn,targ)
        arena.app.taskFinished = True
    ...
    def dracoMeteor(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Special Attack',-2)
        arena.app.taskFinished = True
    ...
    def drainPunch(self,arena,user,targ,earlyExecute=False):
        arena.app.taskFinished = False
        if earlyExecute:
            health = min(user.stats[0],user.hp+int(user.lastDamageDealt/2+1))
            user.healthRestored = health - user.hp
            arena.app.taskFinished = True
            return
        u = ''
    ...
    def extrasensory(self,arena,user,targ,earlyExecute=False):
        if earlyExecute:
            if r.uniform(0,1) > 0.9:
                targ.flinched = True
        arena.app.taskFinished = True
    ...
    def firePunch(self,arena,user,targ,chance=0.9):
        arena.app.taskFinished = False
        if r.uniform(0,1) > chance:
            if targ.types[1].lower() != 'fire' and targ.types[0].lower() != 'fire' and targ.statusCondition == None:
                targ.statusCondition = 'burned'
                u = ''
                f = arena.app.setUserStatus
    ...
    def freezeShock(self,arena,user,targ):
        arena.app.taskFinished = False
        if user.twoTurnMove == None:
            ...
            arena.app.runText(u + user.name + ' became cloaked in a freezing light!')
            arena.app.taskFinished = True
        else:
            user.twoTurnMove = None
            self.thunder(arena,user,targ) ##functionally the same effects- 30% chance of paralysis

    ##G

    def gigaDrain(self,arena,user,targ,earlyExecute=False):
        if earlyExecute:
            health = min(user.stats[0],user.hp+int(user.lastDamageDealt/2+1))
            user.healthRestored = health - user.hp
            arena.app.taskFinished = True
    ...
    def growth(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Attack',1)
        self.statChange(arena,user,'Special Attack',1)
        arena.app.taskFinished = False
    ...
    def hammerArm(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Speed',-1)
        arena.app.taskFinished = True

    def healBell(self,arena,user,targ,earlyExecute=False):
        arena.app.taskFinished = False
        if earlyExecute:
            for x in user.trainer.team:
                if user.trainer.team[x] == None:
                    break
                else:
                    user.trainer.team[x].statusCondition = None
            arena.app.taskFinished = True
            return
        if user == arena.enemyTrainer.team[str(arena.enemyTrainer.active)]:
            ...
            arena.app.setUserStatus()
            u = ''
        arena.app.runText(u+user.name+"'s team had their status ailments removed!")
        arena.app.taskFinished = True
            

    def hibernate(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Defense',2)
        self.statChange(arena,user,'Special Defense',2)
        self.statChange(arena,user,'Attack',-2)
        self.statChange(arena,user,'Special Attack',-2)
        self.statChange(arena,user,'Speed',-2)
        arena.app.taskFinished = True

    def honeClaws(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Attack',1)
        self.statChange(arena,user,'Accuracy',1)
        arena.app.taskFinished = True
    ...
    def iceFang(self,arena,user,targ,earlyExecute=False):
        ...
        if earlyExecute:
            ...
            if r.uniform(0,1) > 0.9:
                if targ.types[1].lower() != 'ice' and targ.types[0].lower() != 'ice' and targ.statusCondition == None:
                    targ.statusCondition = 'freeze'
                    u = ''
                    x = arena.app.setUserStatus
                    if targ.pokemonID == arena.enemyTrainer.team[str(arena.enemyTrainer.active)].pokemonID:
                        u = "Foe's "
                        x = arena.app.setEnemyStatus
                    arena.app.runText(u + targ.name + ' was frozen solid!')
                    x()
        arena.app.taskFinished = True

    def icePunch(self,arena,user,targ,chance=0.9):
        arena.app.taskFinished = False
        if r.uniform(0,1) > chance:
            if targ.types[1].lower() != 'ice' and targ.types[0].lower() != 'ice' and targ.statusCondition == None:
                targ.statusCondition = 'freeze'
    ...
    def magnetRise(self,arena,user,targ):
        arena.app.taskFinished = False
        self.magnetRise = True
        u = "Foe's "
        if targ.pokemonID == arena.enemyTrainer.team[str(arena.enemyTrainer.active)].pokemonID:
            u = ""
        arena.app.runText(u + user.name + " has started floating in the air!")
        arena.app.taskFinished = True
    ...
    def meteorMash(self,arena,user,targ):
        arena.app.taskFinished = False
        if r.uniform(0,1) > 0.8:
            x = self.statChange(arena,targ,'Attack',1,True)
        arena.app.taskFinished = True
        
    def minimize(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Evasion',2)
        user.minimize = True
        arena.app.taskFinished = True
    ...
    def mysticalFire(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Special Attack',-1,True)
        arena.app.taskFinished = True

    ##N

    def nastyPlot(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Special Attack',2)
        arena.app.taskFinished = True
    ...
    def playRough(self,arena,user,targ):
        arena.app.taskFinished = False
        if r.uniform(0,1) > 0.9:
            x = self.statChange(arena,targ,'Attack',-1,True)
        arena.app.taskFinished = True
    ...
    def rapidSpin(self,arena,user,targ,earlyExecute=False):
        arena.app.taskFinished = False
        if earlyExecute:
            if arena.stealthRock[user.trainer]:
                arena.stealthRock[user.trainer] = False
                self.removedStealthRock = True
        else:
            if self.removedStealthRock:
                arena.app.runText('Stealth Rock was spun away!')
                self.removedStealthRock = False
        arena.app.taskFinished = True
    ...
    def rest(self,arena,user,targ,earlyExecute=False):
        arena.app.taskFinished = False
        if earlyExecute:
            health = user.stats[0]
            user.healthRestored = health - user.hp
        ...
        if user.pokemonID == arena.enemyTrainer.team[str(arena.enemyTrainer.active)].pokemonID:
            u = "Foe's "
            f = arena.app.HPUpdateEnemy
        arena.app.runText(u + user.name + ' fell asleep and fully restored its HP!')
        f(user.stats[0],1,0,True)
        f = arena.app.setEnemyStatus
        if targ == arena.enemyTrainer.team[str(arena.enemyTrainer.active)]:
            f = arena.app.setUserStatus
        user.sleepCount = 3
        user.statusCondition = "asleep"
        f()

    def rockPolish(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Speed',2)
        arena.app.taskFinished = True
    ...
    def scald(self,arena,user,targ):
        arena.app.taskFinished = False
        if r.uniform(0,1) > 0.7:
            if targ.types[1].lower() != 'fire' and targ.types[0].lower() != 'fire' and targ.statusCondition == None:
                targ.statusCondition = 'burned'
    ...
    def scaryFace(self,arena,user,targ):
        arena.app.taskFinished = False
    ...
    def shellSmash(self,arena,user,targ):
        arena.app.taskFinished = False
        self.statChange(arena,user,'Attack',2)
        self.statChange(arena,user,'Special Attack',2)
        self.statChange(arena,user,'Speed',2)
    ...
    def sludgeWave(self,arena,user,targ):
        arena.app.taskFinished = False
        if r.uniform(0,1) > 0.9:
            if targ.types[0].lower() not in ['poison','steel'] and targ.types[1].lower() not in ['poison','steel'] and targ.statusCondition == None:
                targ.statusCondition = 'poisoned'
    ...
    def stealthRock(self,arena,user,targ):
        arena.app.taskFinished = False
        if arena.stealthRock[targ.trainer] == False:
            arena.stealthRock[targ.trainer] = True
            arena.app.runText('Rocks were scattered everywhere!')
        else:
            arena.app.runText('But it failed!')
        arena.app.taskFinished = True

    def steelWing(self,arena,user,targ):
        arena.app.taskFinished = False
        if r.uniform(0,1) > 0.9:
            self.statChange(arena,user,'Defense',1)
        arena.app.taskFinished = True

    def stickyWebs(self,arena,user,targ):
        arena.app.taskFinished = False
        if arena.stickyWebs[targ.trainer] == False:
            arena.stickyWebs[targ.trainer] = True
            arena.app.runText('Sticky webs were scattered across the battlefield!')
        else:
            arena.app.runText('But it failed!')
        arena.app.taskFinished = True
    ...
    def synthesis(self,arena,user,targ,earlyExecute=False):
        arena.app.taskFinished = False
        if earlyExecute:
            health = min(user.stats[0],user.hp+int(0.5*user.stats[0]))
            user.healthRestored = health - user.hp
            arena.app.taskFinished = True
            return
        u = ''
        ...
        if user.hp < user.stats[0]:
            arena.app.runText(u + user.name + ' regained some HP!')
            f(min(user.stats[0],user.hp+int(0.5*user.stats[0])),1,0,True)
        else:
    ...
    def thunderPunch(self,arena,user,targ,chance=0.9):
        arena.app.taskFinished = False
        if r.uniform(0,1) > chance:
            if targ.statusCondition == None and targ.types[1].lower() != 'electric' and targ.types[0].lower() != 'electric':
                targ.statusCondition = 'paralyzed'
    ...
    def triAttack(self,arena,user,targ):
        r.choice([self.thunderPunch,self.firePunch,self.icePunch])(arena,user,targ,chance=0.8)
    ...
    def zenHeadbutt(self,arena,user,targ,earlyExecute=False):
        if earlyExecute:
            if r.uniform(0,1) > 0.8:
                targ.flinched = True
        arena.app.taskFinished = True
