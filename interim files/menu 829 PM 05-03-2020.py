...
class menuApp:
    ...
    def loadDraftingMenu(self,count=0):
        if count == 0:
            ...
            self.menuCanvas.create_text(500,400-220,text='<-- Number of Rounds: 2 -->',fill='white',font=self.MenuDPPtFont,tags=('numRounds'))
            self.menuCanvas.create_text(500,500-220,text='<-- Pokemon Per Round: 6 -->' fill='white',font=self.MenuDPPtFont,tags=('pokemonPerRound'))
            self.menuCanvas.create_text(500,600-220,text='START DRAFT',font=self.MenuDPPtFont,tags=('start-draft'))
            master.after(10,genFunc(self.loadDraftingMenu,count+1))
        elif 75 <= count <= 139:
