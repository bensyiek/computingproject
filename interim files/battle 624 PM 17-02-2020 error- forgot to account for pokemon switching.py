        ...
        for x in order:
            ...
            if x[2] == 'user' and self.faintedMidBattle[0]:
                ...
                if not UserFainted and badFlag == 0:
                    ...
                    ##ben please fix
                    ##
                    ##hi i think i've fixed but not sure :P
                    print(x)
...
#############################################################
    ...
    def sendInIfFainted(self):
        ...
        if self.userTrainer.team[str(self.userTrainer.active)].fainted:
            ...
            while True:
                self.emergencyEnemySwitch = str(r.choice(range(5)))
                if self.enemyTrainer.team[str(self.emergencyEnemySwitch)] != None:
                    if not self.enemyTrainer.team[str(self.emergencyEnemySwitch)].fainted:
                        break
...
class move:
    def __init__(self,name,pokemon):
        global moveInfo
        functions = {'Tackle' : self.tackle, 'Growl' : self.growl, 'Leech Seed' : self.leechSeed, 'Vine Whip' : self.vineWhip}
        self.name = name
        self.type = moveInfo[name]['type']
        self.category = moveInfo[name]['category']
        self.description = moveInfo[name]['description']
