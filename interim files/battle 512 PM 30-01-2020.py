...
pkmnInfo = {
    'Charizard' : {'stats' : (45,49,49,65,65,45), 'type1' : 'Grass', 'type2' : 'Poison', 'ability' : ['Overgrow'], 'moves' : ['Tackle','Tackle','Tackle','Tackle']}
    }
natures = [
...
class Arena:
    ...
    def processTurn(self,userInput,enemyInput):
        ...
        if userInput[0] == 'attack' and enemyInput[0] == 'attack': ## need to decide which goes first
            ...
            if userInput[1].priority > enemyInput[1].priority: 
                ...
                if userTrainer.team[str(self.userTrainer.active)].stats[5] > enemyTrainer.team[str(self.enemyTrainer.active)].stats[5]:
                    order = (userInput,enemyInput)
                elif userTrainer.team[str(self.userTrainer.active)].stats[5] < enemyTrainer.team[str(self.enemyTrainer.active)].stats[5]:
                    order = (enemyInput,userInput)
        ...
        for x in order:
            ...
            if x[2] == 'user' and self.faintedMidBattle[0]:
                ...
                if x[2] == 'user': ## I could compress k=genfunc(...); q.add(k) into one line but they are kept separate for clarity
                    ...
                    k = genFunc(self.app.runText,r.choice(userSwitchMessages) % self.findActiveName(userTrainer))
                    q.add(k)
                    q.add(self.app.switchUser)
                    k = genFunc(self.setUserActive,x[1])
                    q.add(k)
                    k = genFunc(self.app.runText,r.choice(userSendInMessages) % self.findActiveName(userTrainer))
                    q.add(k)
                ...
                if x[2] == 'user':
                    user = self.userTrainer.team[str(self.userTrainer.active)] ##user here refers to the pokemon using the move
                    targ = self.enemyTrainer.team[str(self.enemyTrainer.active)]
                    UserFainted = self.faintedMidBattle[0]
                    j = "Foe's "
                else:
                    user = self.enemyTrainer.team[str(self.enemyTrainer.active)]
                    targ = self.userTrainer.team[str(self.userTrainer.active)]
                    UserFainted = self.faintedMidBattle[1]
                    j = ""
                if user.flinched == True:
                    k = genFunc(self.app.runText,j + user.name + ' flinched!')
                    q.add(k)
                    badFlag = 1 ##they can't do their thing
                elif user.statusCondition == 'paralyzed':
                    if r.choice([0,1]) == 1:
                        k = genFunc(self.app.runText, j + self.findActiveName(user) + ' is paralyzed and cannot move!')
                        q.add(k)
                        badFlag = 1
                if not UserFainted and badFlag == 0:
                    print(x)
                    k = genFunc(self.app.runText,j + user.name + ' used ' + x[1].name + '!')
                    q.add(k)
                        ...
                        if x[1].category != 'Status':
                            ...
                            if damage[1] == 2:
                                ...
                                if damage[0] >= targ.hp:
                                    ...
                                    #q.add(k)
                                    k = genFunc(user.takeDamage,damage[0])
                                    q.add(k)
                                    if damage[1] == 1:
    ...
    def executeTurn(self,q,textWasRunning=False,EOT=False):
        if self.app.taskFinished and not self.app.textRunning and not textWasRunning:
            if len(q.q) == 0:##used for two things. once when executing the main turn,
                            ##and second when executing the knockouts at EOT
                if not EOT:
                    self.endOfTurn()
            else:
                q.rem()() ##executes whatever q returns
        if textWasRunning:
            k = genFunc(self.executeTurn,q,self.app.textRunning,EOT)
...
##EOT FUNCS HERE
    ...
    def endOfTurn(self,i=0,numTasks=0):
        ...
        if i == len(self.endOfTurnQueue.q):
            ...
            if numTasks[0] == self.app.completeProcesses:
                numTasks = numTasks[1]()
    ...
    def sendInIfFainted(self):
        ...
        elif self.enemyTrainer.team[str(self.userTrainer.active)].fainted:
            while True:
                self.emergencyEnemySwitch = r.choice(range(5))
                if self.enemyTrainer.team[self.emergencyEnemySwitch] != None:
                    if not self.enemyTrainer.team[self.emergencyEnemySwitch].fainted:
                        break
            q.add(self.checkEnemySwitch)
        self.executeTurn(q,EOT=False)

    def checkUserSwitch(self,reset=0):
        self.app.taskFinished = False
        if not reset and self.emergencyUserSwitch == '':
            master.after(50,self.checkUserSwitch)
            return
        elif reset:
            self.emergencyUserSwitch = ''
            return
    ...
    def checkEnemySwitch(self,reset=0):
        ...
        if not reset and self.emergencyEnemySwitch == '':
            master.after(50,checkEnemySwitch)
            return
        elif reset:
            self.emergencyEnemySwitch = ''
            return
        ...
        k = genFunc(self.checkEnemySwitch,1)
        master.after(50,k)
...
class move:
    def __init__(self,name,pokemon):
        global moveInfo
        functions = {'Tackle' : self.tackle, 'Growl' : self.growl,
                     'Leech Seed' : self.leechSeed, 'Vine Whip' : self.vineWhip}
        self.name = name
        self.type = moveInfo[name]['type']
        self.category = moveInfo[name]['category']
        self.description = moveInfo[name]['description']
    ...
    def growl(self,arena,user,targ):
        ...
        if x == 'ALLOK':
            ...
            arena.app.runText('But ' + targ.name + "'s attack can't go any " + x + '!')
        arena.app.taskFinished = False
