...
class draftingApp:
    def __init__(self,master,draftingArena=False):
        ...
        self.viewChoicesButton = Button(self.pokedexFrame,state=DISABLED,bg='#9fe8c4',font=self.DPPtFont2,text='View Choices',command=self.showChoices,cursor='hand2')
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def slideChoicesAcross(self,loop=0):
        if loop == 0:
            ...
            for x in range(0,18):
                if x == 15:
                    xOffset = 96+7
                self.battleCanvas.create_rectangle(0+xOffset+490,0+yOffset,90+xOffset+490,90+yOffset,fill='light grey', outline='grey',width=2,tags=('choices-data'))
            ...
            yOffset = 5
            count = 0
            print(self.pokemonChosen)
            for pkmn in self.pokemonChosen:
                if count == 15:
                    xOffset = 96+7
                self.tempChoicesPhotos.append(ImageTk.PhotoImage(Image.open(pokemon[pkmn.species])))
                ID = self.battleCanvas.create_image(0+xOffset+490,0+yOffset,anchor=NW,image=self.tempChoicesPhotos[count],tags=('choices-data'))
                xOffset = (xOffset + 96) % 480
                yOffset = (yOffset) + 96*int((x%5+1)/5)
                self.tempChoicesIDs.append(ID)
                count += 1
            self.battleCanvas.lift('choices-data')
            master.after(5,genFunc(self.slideChoicesAcross,loop+1))
        elif 98 >= loop >= 1:
            self.battleCanvas.move('choices-data',-5,0)
            master.after(10,genFunc(self.slideChoicesAcross,loop+1))
        else:
            self.viewChoicesButton.config(bg='#ff9d9d',text='Hide Choices',command=self.hideChoices,state=ACTIVE)
            count = 0
            for ID in self.tempChoicesIDs:
                self.battleCanvas.tag_bind(ID,'<Enter>',genFunc(self.choices_hoverOver,self.pokemonChosen[count],ID))
                self.battleCanvas.tag_bind(ID,'<Leave>',self.choices_hoverOff)
    ...
    def choices_boxSelection(self,coords,offset=0,sign=0.1,posOrNeg=-0.02): ##pops up a small green box around the user's choice
        ...
        self.battleCanvas.create_image(coords[0]-offset,coords[1]-offset,image=self.choices_boxImageTL,anchor=S,tags=('selection-box'))
        self.battleCanvas.create_image(coords[0]+90+offset,coords[1]-offset,image=self.choices_boxImageTR,anchor=S,tags=('selection-box'))
        self.battleCanvas.create_image(coords[0]-offset,coords[1]+90+offset,image=self.choices_boxImageBL,anchor=S,tags=('selection-box'))
        self.battleCanvas.create_image(coords[0]+90+offset,coords[1]+90+offset,image=self.choices_boxImageBR,anchor=S,tags=('selection-box'))
        if int(sign*10) == -1:
            posOrNeg = 0.02
        elif int(sign*10) == 0.1:
            posOrNeg = -0.02
    ...
    def hideChoices(self):
        for ID in self.tempChoicesIDs:
            ...
            self.battelCanvas.tag_unbind(ID,'<Leave>')
        self.deactivateButtons()
        self.slideChoicesOut()

    def slideChoicesOut(self,loop=0):
        if loop <= 97:
            self.battleCanvas.move('choices-data',5)
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def activateButtons(self):
        ...
        for coords in self.pokemonCoordsSouth:
            if self.pokemonAvailable[str(coords)] != None:
                ...
                break
        self.viewChoicesButton.config(state=ACTIVE)
    ...
    def deactivateButtons(self):
        ...
        ##
        ##
        ## also, breaking up code with these taskfinisheds has allowed me to make the code more modular
        ## as we can call each function without worrying if it will call another function unintentionally
        ##
        ##
        ##
        ##
        ...
        self.viewChoicesButton.config(state=DISABLED)
