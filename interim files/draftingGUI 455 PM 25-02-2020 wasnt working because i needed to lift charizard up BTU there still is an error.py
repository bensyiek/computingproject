...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def disableChoices(self):
        for ID in self.tempChoicesIDs:
            self.battleCanvas.tag_unbind(ID,'<Enter>')
            self.battleCanvas.tag_unbind(ID,'<Leave>')
            self.battleCanvas.tag_unbind(ID,'<Button-1>')
        for x in range(0,4):
    ...
    def enableChoices(self):
        ...
        for x in range(0,4):
            ...
            if self.team[x] != None:
                print('bound!')
                self.battleCanvas.tag_bind('team-'+str(x),'<Button-1>',genFunc(self.removeFromTeam,x))
    ...
    def removeFromTeam(self,x):
        ...
        self.disableChoices()
        self.team[x] = None
        self.moveTeamMemberOffscreen(self,self.teamToChoicesLink[x],x)
        self.teamToChoicesLink[x] = None
    ...
    def addToTeam(self,x):
        for i in range(4):
            if self.team[i] == None:
                ...
                self.teamImages[i] = (ImageTk.PhotoImage(Image.open(pokemon[self.pokemonChosen[x].species]).resize((100,100))))
                
                self.teamIDs[i] = self.battleCanvas.create_image(15+120*i,537-125,anchor=NW,image=self.teamImages[i],tags=('team-'+str(i)))
    ...
    def moveTeamMemberOffscreen(self,IDChoice,i,loop=0):
        if loop < 25:
            ...
            print(c)
            self.battleCanvas.delete('transparencyBox-choice-'+str(IDChoice)) ##rest is as above
            self.transparentBoxImage2[IDChoice] = ImageTk.PhotoImage(Image.open('assets/drafting/greyRectangleTransparent/grey'+str(25-1-loop)+'.png').resize((90,90)))
            self.battleCanvas.create_image(c[0],c[1],anchor=NW,image=self.transparentBoxImage2[IDChoice],tags=('transparencyBox-choice-'+str(IDChoice)))
