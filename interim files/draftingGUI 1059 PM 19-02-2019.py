...
class draftingApp:
    ...
    def initResources(self):
        ...
        self.battleCanvas.create_image(115,150,image=self.platformImage,tags=('platform','platformC1','platformR1','platformC1R1'))
        self.battleCanvas.create_image(115,320,image=self.platformImage,tags=('platform','platformC1','platformR2','platformC1R2'))
        self.battleCanvas.create_image(115,490,image=self.platformImage,tags=('platform','platformC1','platformR3','platformC1R3'))
        self.battleCanvas.create_image(375,130,image=self.platformImage,tags=('platform','platformC2','platformR1','platformC2R1'))
        self.battleCanvas.create_image(375,300,image=self.platformImage,tags=('platform','platformC2','platformR2','platformC2R2'))
        self.battleCanvas.create_image(375,470,image=self.platformImage,tags=('platform','platformC2','platformR3','platformC2R3'))
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def activateButtons(self):
        ...
        for coords in self.pokemonCoordsSouth:
            if self.pokemonAvailable[str(coords)] != None:
                k = genFunc(self.testVars,coords)
                self.battleCanvas.tag_bind(self.pokemonCanvasItemVars[str(coords)],'<Button-1>',k)
                self.battleCanvas.tag_bind(self.pokemonCanvasItemVars[str(coords)],'<Enter>',self.handCursor)
    ...
    def arrowCursor(self,event):
        self.battleCanvas.config(cursor='arrow')

    def deactivateButtons(self,event):
    ...
    def selectPokemon(self,coords):
        ...
        q = battle.serviceQueue()
        q.add([False,genFunc(self.runText,'You selected ' + self.pokemonAvailable[str(coords)]+'!')])
        ...
        q.add([self.pokemonCoordsSouth[self.numberLeftInRound-1],self.processPokemonRetrieval])
        q.add([style,self.movePlatforms,'off',style])
        q.add([False,self.draft])
        self.executeTurn(q)

    def processPokemonRetrieval(self):
        count = 0
        for coords in self.pokemonCoordsSouth:
            self.taskFinished[str(coords)] = False
            if self.pokemonAvailable[str(coords)] != None:
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def switchEnemy(self,tick=0,count=0,coords=False):
        ...
        if tick == 0:
            ...
            k = genFunc(self.switchEnemy,tick+1,count,coords)
            self.master.after(5,k)
        elif 1 <= tick <= 96:
            ...
            self.master.update()
            k = genFunc(self.switchEnemy,tick+1,1,coords)
            self.master.after(5,k)
            ...
            self.battleCanvas.create_image(coords[0],coords[1],image=self.EpokeballImage[str(coords)],anchor=S,tags=('enemy-pokeball'+str(coords)))
            self.battleCanvas.delete('enemy-pokemon'+str(coords))
            k = genFunc(self.switchEnemy,tick,count+1,coords)
    ...
    def movePlatforms(self,direction,style=False,loop=0,firstTime=True):
        if direction == 'off': 
            ...
            if style == 'columnSeparation': ##the columns move apart in opposite directions
                ...
                self.battleCanvas.move('platformC2',0,-10)##as above but for the other
                if loop == 52:
                    if firstTime:
