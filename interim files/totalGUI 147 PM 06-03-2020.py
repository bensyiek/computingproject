...
class menuApp:
    def __init__(self,master):
        ...
        self.menuDistances = {'main':500,'drafting':610}
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def startBattle(self):
        ...
        enemyTrainer = battle.trainer(r.choice(names),randClass,'Blue')
        for x in range(4):
            ...
            if bulky > physical and bulky > special:
                if numPhysical + numSpecial <= 1:
                    P.evs = r.choice(EVSpreads['pure stall'])
                elif numPhysical >= 2:
                    P.evs = r.choice(EVSpreads['physical wall'])
                elif numSpecial >= 2:
                    P.evs = r.choice(EVSpreads['special wall'])
                else:
                    P.evs = r.choice(EVSpreads['mixed wall'])
            elif special > physical and special > bulky:
                if numPhysical == 0:
                    P.evs = r.choice(EVSpreads['special sweeper'])
                elif numPhysical >= 1:
                    P.evs = r.choice(EVSpreads['mixed attacker'])
            elif physical > special and physical > bulky:
                if numSpecial == 0:
                    P.evs = r.choice(EVSpreads['physical sweeper'])
                elif numSpecial >= 1:
                    P.evs = r.choice(EVSpreads['mixed attacker'])
            elif physical == special and physical > bulky:
                P.evs = r.choice(EVSpreads['mixed attacker'])
            else:
                P.evs = r.choice(EVSpreads['true balance'])
            P.statCalculate()
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def EOTSwitchButtons(self): ##these are used if a pokemon faints. User must switch in at end of turn.
        ...
        for button in self.switchButtons:
            ...
            if count != self.arena.userTrainer.active and self.arena.userTrainer.team[str(count)] != None:
                print(button)
    ...
    def receiveUserInput(self,userInput):
        ...
        if self.arena.enemyTrainer.powerLevel == 'regular':
            ...
            for move in enemyActive.moves:
                if enemyActive.switchedInThisTurn:
                    ...
                    if move[0].name == 'Fake Out':
                        ...
                        if move[0].category != 'Status' and arena.typeModifiers(enemyActive,userActive,move[0])[1] == 2:
                            enemyInput = ('attack',self.arena.enemyTrainer.team[str(self.arena.enemyTrainer.active)].moves[moveCount][0],'enemy')
                            break
                moveCount += 1
            ...
            moveCount = 0
            for move in enemyActive.moves:
                if move[0].category != 'Status' and arena.typeModifiers(enemyActive,userActive,move[0])[1] == 1:
                    enemyInput = ('attack',self.arena.enemyTrainer.team[str(self.arena.enemyTrainer.active)].moves[moveCount][0],'enemy')
                moveCount += 1
