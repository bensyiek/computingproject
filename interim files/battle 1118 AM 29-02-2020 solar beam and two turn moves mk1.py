        ...
        for x in order:
            ...
            if x[2] == 'user' and self.faintedMidBattle[0]:
                ...
                if not UserFainted and badFlag == 0:
                    ...
                    q.add(k)
                    if move in ['Solar Beam'] and user.twoTurnMove == None:
                        k = genFunc(x[1].function,self,user,targ)
                        q.add(k)
                    elif self.doesMoveHit(x[1],user,targ):
                        for hit in range(0,loop):
                            if x[1].category != 'Status':
                                ...
                                if damage[1] == 0:
                                    ...
                                    if x[1].name in ['Waterfall']:
                                        x[1].function(self,user,targ)
...
class move:
    def __init__(self,name,pokemon):
        ...
        functions = {
            ...
            'Aqua Jet':self.aquaJet,
            'Aqua Tail':self.aquaTail,

            #C
            'Crunch':self.crunch,
            'Chip Away':self.chipAway,
    ...
    def icePunch(self,arena,user,targ):
        ...
        if r.uniform(0,1) > 0.9:
            if targ.types[1].lower() != 'ice' and targs.types[0].lower() != 'ice' and targ.statusCondition == None:
                ...
                arena.runText(u + targ.name + ' was frozen solid!')
                x()
        arena.app.taskFinished = True
        ...
        if user.twoTurnMove == None:
            ...
            for move in pokemon.moves:
                if move.name == 'Solar Beam':
                    user.twoTurnmove = ('attack',user,move)
                    break
            arena.app.runText(u + pokemon.name + ' is absorbing light!')
        else:
            user.twoTurnMove = None
        arena.app.taskFinished = True
