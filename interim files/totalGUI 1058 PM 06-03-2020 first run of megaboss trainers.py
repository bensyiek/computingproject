...
class menuApp:
    def __init__(self,master):
        ...
        self.menuDistances = {'main':470,'drafting':610}
...
#-----------------------------------------------------------------------------------------------------------#
    ...
    def loadRandomBattle(self):
        ...
        for x in range(12):
            if x == 6:
                Trainer = userTrainer
            P = battle.pokemon(r.choice(pokemonInDraft),r.randint(50,60),Trainer)
            Trainer.addToTeam(x%6)
            physical = 0
            numPhysical = 0
            special = 0
            numSpecial = 0
            ...
            P.statCalculate()
        switchToBattleGUI(userTrainer,enemyTrainer,self.master,[[],[0,0]])
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def startBattle(self):
        ...
        for x in [self.textFrame,self.battleFrame,self.pokedexFrame]:
            x.grid_forget()
        if self.battlesLeft == 1 and self.numBattles == 10 or True:
            enemyTrainer = self.findBossBattleTrainer()
            switchToBattleGUI(userTrainer,enemyTrainer,self.master,[self.pokemonChosen,[self.battlesLeft,self.numBattles]],megaboss=True)
        else:
    ...
    def findBossBattleTrainer(self):
        trainer = r.choice(finalBoss)
        if trainer == 'Ben':
            enemyTrainer = trainer('Ben','Developer','Ben')
            pokemon = r.sample(['Gardevoir','Magnezone','Chandelure','Cloyster','Ferrothorn'],k=3)
            finalizedPokemon = []
            x = 0
            for P in pokemon:
                PKMN = battle.pokemon(P,random.randint(57,63),enemyTrainer)
                P.iv = (31,31,31,31,31,31)
                if P == 'Gardevoir':
                    P.forceMoves(['Moonblast','Psychic','Wish','Thunderbolt'])
                    P.ev = [0,0,0,252,4,252]
                elif P == 'Magnezone':
                    P.forceMoves(['Reflect','Thunder Wave','Thunderbolt','Flash Cannon'])
                    P.ev = [0,0,100,252,158,0]
                elif P == 'Chandelure':
                    P.forceMoves(['Minimize','Shadow Ball','Flamethrower','Energy Ball'])
                    P.ev = [0,0,0,252,4,252]
                elif P == 'Cloyster':
                    P.forceMoves(['Shell Smash','Pin Missile','Liquidation','Icicle Spear'])
                    P.ev = [0,252,4,0,0,252]
                elif P == 'Ferrothorn':
                    P.forceMoves(['Iron Defense','Power Whip','Gyro Ball','Iron Head'])
                    P.ev = [4,252,252,0,0,0]
                    P.iv = (31,31,31,31,31,0)
                P.statCalculate()
                enemyTrainer.addToTeam(x)
                x+=1
            PKMN = battle.pokemon('Kyurem-B',65,enemyTrainer)
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def draft(self):
        ...
        if self.roundsLeft > 2 or self.roundsLeft == 0:                                                                                                         ##this is to take out the s of rounds if we aren't taking  about a plural
...
class app:
    def __init__(self,master,arena,receiveInfo):
    ...
    def endBattle(self,result,count=-1):
        if count == -1:
            ...
            if self.numBattles[0] > 0:
                self.battleCanvas.create_text(240,320,text='Battles Left: '+str(self.numBattles[0]),fill='white',font=self.DPPtFont,tags=('enter-text'))
            else:
    ...
    def returnToDraft(self,result,count=0):
        if count == 0:
            ...
            if result == 'lose' or self.numBattles[0] == 0:
                self.switchToMenuGUI(master)
            elif result == 'win':
                self.returnToDraftingGui([self.pokemonChosen,self.numBattles],master)##ben the relevant information needs to included-i.e. receiveInfo
