    ...
    def slideChoicesAcross(self,loop=0,teamCreation=False): ##this function is also used for loading team creation.
        ...
        if loop == 0:
            ...
            for x in range(0,18):
                if x == 15:
                    xOffset = 96+7
                self.boxIDs.append(self.battleCanvas.create_rectangle(0+xOffset+490,0+yOffset,90+xOffset+490,90+yOffset,fill='light grey', outline='grey',width=2,tags=('choices-data')))
            ...
            if teamCreation: ##only applicable if team creation is in use- i.e. at the end of a draft
                xOffset = 15
                for x in range(0,4):
                    self.battleCanvas.create_rectangle(0+xOffset+490,537-125,100+xOffset+490,537-25,fill='light grey',outline='grey',width=2,tags=('choices-data'))
                    xOffset += 120
            ...
            for x in range(len(self.tempChoicesIDs)):
                self.battleCanvas.tag_bind(self.tempChoicesIDs[x],'<Enter>',genFunc(self.choices_hoverOver,self.pokemonChosen[count],self.tempChoicesIDs[x]))
                self.battleCanvas.tag_bind(self.tempChoicesIDs[x],'<Leave>',self.choices_hoverOff)
                if teamCreation:
                    self.battleCanvas.tag_bind(self.tempChociesIDs[x],'<Button-1>',genFunc(self.addToTeam,x))
...
#------------------------------------------------------------------------------------------------------------------------------#
           # Team selection. These are where the functions that let the user pick and EV train their team are.
#------------------------------------------------------------------------------------------------------------------------------#

    def initTeamCreation():
    ...
    def enableChoices(self):
        ...
        count = 0
        for x in range(len(self.tempChoicesIDs)):
            self.battleCanvas.tag_bind(self.tempChoicesIDs[x],'<Enter>',genFunc(self.choices_hoverOver,self.pokemonChosen[count],self.tempChoiceIDs[x]))
            self.battleCanvas.tag_bind(self.tempChoicesIDs[x],'<Leave>',self.choices_hoverOff)
            self.battleCanvas.tag_bind(ID,'<Button-1>',genFunc(self.addToTeam,x))
        ...
        if len(self.team) < 4:
            for i in range(4):
                if self.team[i] != None:
                    ...
                    self.tempChoicesIDs[x].config(state=DISABLED)
                    self.teamImages.append(ImageTk.PhotoImage(Image.open(pokemon[self.pokemonChosen[x].species])))
                    k = self.battleCanvas.create_image(15+120*x,537,anchor=NW,image=self.teamImages[-1],tags=('team-'+str(x)))
                    self.disableChoices()
                    self.moveTeamMemberOnscreen(k)

    def moveTeamMemberOnscreen(self,ID,loop=0):
        if loop < 25:
            self.battleCanvas.move(ID,0,-5)
            master.after(20,genFunc(self.moveTeamMemberOnscreen,ID,loop+1))
        else:
            self.battleCanvas.move(ID,0,-2)
            self.enableChoices()
#------------------------------------------------------------------------------------------------------------------------------#
