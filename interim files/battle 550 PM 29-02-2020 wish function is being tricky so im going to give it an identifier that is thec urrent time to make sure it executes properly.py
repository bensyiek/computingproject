...
class Arena:
    def __init__(self,trainer1,trainer2):
        ...
        self.wish = {self.userTrainer : 0, self.enemyTrainer : 0}
        self.yawn = {}
    ...
    def doesMoveHit(self,move,user,targ):
        x = move.accuracy * accStages[str(user.accuracy)]
                            ...
                            if self.doesMoveHit(x[1],user,targ):
                                ...
                                if x[1].category != 'Status':
                                    ...
                                    if damage[1] == 0:
                                        ...
                                        if x[1].name in ['Clear Smog','Fake Out']:
...
################################################################################
    ...
    def wishEOT(self,trainer):
        if self.wish[trainer] == 0:
            return False
        self.wish[trainer] -= 1
        if self.wish[trainer] == 0:
            if 
            arena.app.runText('The wish came true!')
            u = ''
            f = self.HPUpdateUser
            if trainer == self.enemyTrainer:
                u = "Foe's "
                f = self.HPUpdateEnemy
            if trainer.team[str(trainer.active)].hp == trainer.team[str(trainer.active)]:
                arena.app.runText(u+trainer.team[str(trainer.active)].name + "'s HP is already full!")
            else:
                p = trainer.team[str(trainer.active)]
                p.hp = min(p.hp+int(p.stats[0]/2),p.stats[0]/2)
                self.HPUpdateUser
...
#############################################################
    ...
    def typeModifiers(self,user,target,move):
        ...
        chance = 16
        if move.name in ['Blaze Kick','Slash']:
            chance = 8
...
class move:
    def __init__(self,name,pokemon):
        ...
        moveSpecifier = {'Bullet Seed' : 'multihit','Dual Chop' : 'twohit'}#,'Earthquake':'regular','Growl' : 'regular','Tackle' : 'regular'}
        self.name = name
    ...
    def fakeOut(self,arena,user,targ,earlyExecute=True):
        if earlyExecute:
            targ.flinched = True
        arena.app.taskFinished = True
    ...
    def wish(self,arena,user,targ):
        arena.app.taskFinished = False
        if self.wish[user.trainer] == 0:
            arena.app.endOfTurnQueue.add(genFunc(arena.wishEOT,user.trainer))
        else:
            arena.app.runText('But it failed!')
        arena.app.taskFinished = True
