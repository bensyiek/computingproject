            ...
            ##########SUCKER PUNCH CHECK##########
                if userInput[1].name == 'Sucker Punch' and enemyInput[1].name != 'Sucker Punch' and enemyInput[1].category in ['Physical','Special']:
                    ...
                    if userSpeed > enemySpeed:
                        ...
                        self.enemyTrainer.team[str(self.enemyTrainer.active)].suckerPunchFail = False
                    ##speed tie sucker punches are decided down below
            ########END SUCKER PUNCH CHECK########

                        
                if userSpeed > enemySpeed:
                    if userInput[1].name == 'Roar' and enemyInput[1].name == 'Roar':
                            ...
                            if self.doesMoveHit(x[1],user,targ):
                                ...
                                if x[1].category != 'Status':
                                    ...
                                    if damage[1] == 0:
                                        ...
                                        if x[1].name in ['Clear Smog','Dark Pulse','Dragon Rush','Drain Punch','Fake Out','Fire Fang','Giga Drain','Ice Fang','Iron Head','Rock Slide','Thunder Fang','Waterfall','Zen Headbutt']:
                                        ...
                                        if damage[0] >= targ.hp-loopDamage+targ.healthRestored: ##loop damage is for looping moves that deal damage across multiple hits, healthRestored is for moves like rest drainpunch or gigadrain that if not accounted for might incorrectly call a KO animation
...
#############################################################
    ...
    def typeModifiers(self,user,target,move):
        ...
        chance = 16
        if move.name in ['Blaze Kick','Cross Poison','Night Slash','Shadow Claw','Slash','Stone Edge']:
            chance = 8
...
class move:
    def __init__(self,name,pokemon):
        ...
        moveSpecifier = {'Bone Rush' : 'multihit','Dual Chop' : 'twohit','Icicle Spear':'multihit','Rock Blast':'multihit'}
        self.name = name
    ...
    def drainPunch(self,arena,user,targ,earlyExecute):
        arena.app.taskFinished = False
        if earlyExecute:
