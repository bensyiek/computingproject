...
class app:
            ...
            if len(temp) > 9:
                ...
                for x in range(0,len(temp)):
                    if x == len(temp)-1:
                        ...
                        newTemp += temp[x] + '\n'
                temp = newTemp
            k = genFunc(self.receiveUserInput, ['move',count,'user'])
    ...
    def receiveUserInput(self,userInput):
        self.disableButtons()
        if userInput[0] == 'move':
            user[1] = self.arena.userTrainer.team[str(self.arena.userTrainer.active)].moves[user[1]][0]
        user[1] = tuple(user[1])
        enemyInput = r.choice(['switch','attack'])
        if enemyInput == 'attack':
            move = r.randint(0,4)
            enemyInput = ('switch',self.arena.enemyTrainer.team[str(self.arena.enemyTrainer.active)].moves[move][0],'enemy')
...
# The following are functions pertaining to the initialize of battles
    ...
    def enemyPokemonSendOut(self,count=0,ready=0):
        ...
        if count == 0 and not ready:
            ...
            print(self.arena)
            print(self.arena.enemyTrainer)
            print(self.arena.findActiveName(self.arena.enemyTrainer))
