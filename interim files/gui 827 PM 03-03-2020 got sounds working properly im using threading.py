...
for x in range(5):
    battle.pokemon('Chandelure',random.randint(50,60),trainerTwo)
...
##ex. fainting
    ...
    def HPUpdateEnemy(self,newHP,start=1,flashing=0,updateInGui=False):
        if start:
            self.taskFinished = False
    ...
    def HPUpdateUser(self,newHP,start=1,flashing=0,updateInGui=False):
        if start:
            print('started')
            self.taskFinished = False
            self.lastRatio = self.printedUserHP /self.UserMaxHP
...
gameRunning = True
def backgroundBattleMusic():
    while gameRunning:
        playsound('music/vs-trainer.wav')

App = app(master,arena)
T = threading.Thread(target=backgroundBattleMusic)
T.start()
#winsound.PlaySound(random.choice(battleMusic),winsound.SND_ASYNC|winsound.SND_LOOP|winsound.SND_NOSTOP)
mainloop()
