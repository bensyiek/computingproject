    ...
    def endOfTurn(self,i=0,numTasks=0):
        ...
        if i == len(self.endOfTurnQueue.q) and numTasks == self.app.processesComplete:
            ...
            for item in self.removeList:
                del self.endOfTurnQueue.q[self.removeList[item]]
                #if x == False:
...
#############################################################
    ...
    def findDamage(self,user,target,move):
        ...
        typeModifiers = self.ty[eModifiers(user,target,move)
        if typeModifiers[0] == 0:
            return (0,0)
        damage = int(int(((((2 * int(user.level))/5 + 2) * int(move.power) * user.stats[1]/target.stats[2])/50 * combinedModifier + 2))) * typeModifiers * r.uniform(0.95,1.05)) + 1
        print(damage)
