...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def tablet_displayMove(self,move):
        ...
        if move.category != 'Status':
            ...
            count += 1
        self.tabletCanvas.create_text(20,300+count*30,text = 'Accuracy: ' + str(move.accuracy),font=self.tabletFont,anchor=W,tags=('tablet-data','tablet-move-accuracy'))
...
#------------------------------------------------------------------------------------------------------------------------------#
    ...
    def loadButtonsFirstTime(self):
        ...
        for count in range(4):
            self.moveButtons.append(Button(self.buttonFrame,bg='white',text=' ',width=9,height=3,cursor='hand2',font=self.DPPtFont2,state=DISABLED))
    ...
    def useMove(self,elementalType,targ,tick=0,leftOffAt=-1):
        ...
        if tick == 0:
            ...
            if elementalType.lower() in ['ground','normal','poison','ice','bug','fighting']:##and these vary based on how they look when they fade out
                self.moveE = 'return'
            if elementalType.lower() in ['dragon','poison','dark','flying','ice','electric','fighting']: ##these moves need to operate faster
                self.delay = 50
